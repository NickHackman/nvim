  ...
  describe("opts", function()
    it("two descriptions throws", function()
      assert.error_matches(
        ...
        "expected only a single description, but got two. First `description one` then `description two`"
      )
    end)
    ...
    it("unsupported vararg", function()
      assert.error_matches(
        ...
        "expected option to be a string or function, but got `1` at index `1`"
      )
    end)

    it("valid", function()
      local actual = keymap.opts(
        "description",
        keymap.noremap(),
        keymap.nowait(),
        keymap.expr()
